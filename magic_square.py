...
def is_acceptable(board: list[list[int]], n: int):
    ...
    for row in board:
        if min(row) != 0 and sum(row) != expected_sum:
            return False

    for column in [[row[i] for row in board] for i in range(n)]:
        if min(column) != 0 and sum(column) != expected_sum:
            return False
    diag45 = [board[i][i] for i in range(n)]
    if min(diag45) != 0 and sum(diag45) != expected_sum:
        return False

    diag135 = [board[i][n-1-i] for i in range(n)]
    if min(diag135) != 0 and sum(diag135) != expected_sum:
        return False
